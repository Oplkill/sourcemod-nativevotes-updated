...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
    ...
    {
        FormatEx(error, err_max, "Unsupported game: %s", engineName);
...
{
    for (int i, size = myArray.Length; i < size; ++i)
    {
...
bool DoClientVote(NativeVote vote, int[] clients, int num_clients)
{
    ...
    for (int i; i < num_clients; ++i)
    {
        ...
        {
            realClients[totalPlayers++] = client;
        }
    }
...
bool Internal_RedrawToClient(int client, bool revotes)
{
    ...
    FormatEx(revotePhrase, sizeof revotePhrase, "%T", "NativeVotes Revote", client);
...
public int Native_Create(Handle plugin, int numParams)
{
    ...
    NativeVote vote = Data_CreateVote(voteType, actions);
...
public int Native_Display(Handle plugin, int numParams)
{
    ...
    return StartVote(vote, count, clients, GetNativeCell(4), flags);
}
...
public int Native_GetTitle(Handle plugin, int numParams)
{
    ...
    return SetNativeString(2, title, len) == SP_ERROR_NONE;
}
