...
///[clientId][nominatedMapName]
StringMap g_mapClientNominatedMaps = null;

///[mapName][numberNominationsForThatMap]
StringMap g_mapMapNominationsCount = null;
...
public void OnPluginStart()
{
    ...
    g_mapClientNominatedMaps = new StringMap();
    g_mapMapNominationsCount = new StringMap();
}
...
public void OnAllPluginsLoaded()
{
    ...
    g_NativeVotes = LibraryExists(LIBRARY) && GetFeatureStatus(FeatureType_Native, "NativeVotes_AreVoteCommandsSupported") == FeatureStatus_Available && NativeVotes_AreVoteCommandsSupported();
...
{
    if (StrEqual(name, LIBRARY, false) && GetFeatureStatus(FeatureType_Native, "NativeVotes_AreVoteCommandsSupported") == FeatureStatus_Available && NativeVotes_AreVoteCommandsSupported())
    {
        g_NativeVotes = true;
        RegisterVoteHandler();
    }
...
{
    g_mapClientNominatedMaps.Clear();
    g_mapMapNominationsCount.Clear();
}
...
public Action Command_Addmap(int client, int args)
{
    ...
    SetClientVoteNomination(client, mapname);
...
public int Handler_MapSelectMenu(Menu menu, MenuAction action, int param1, int param2)
{
    ...
    case MenuAction_Select:
    {
        ...
        SetClientVoteNomination(param1, map);
    }
    ...
    case MenuAction_DisplayItem:
    {
        ...
        if (g_mapMapNominationsCount.GetValue(map, numVotes))
        {
            Format(display, sizeof(display), "%s (votes: %d)", displayName, numVotes);
            return RedrawMenuItem(display);
        }
...
public void SetClientVoteNomination(int client, const char[] mapName)
{
    char clientBuffer[PLATFORM_MAX_PATH];
    IntToString(client, clientBuffer, PLATFORM_MAX_PATH);

    // if client had prev nomination
    if (g_mapClientNominatedMaps.ContainsKey(clientBuffer))
    {
        char prevNominatedMap[PLATFORM_MAX_PATH];
        g_mapClientNominatedMaps.GetString(clientBuffer, prevNominatedMap, PLATFORM_MAX_PATH);

        if (g_mapMapNominationsCount.ContainsKey(prevNominatedMap))
        {
            int numNominations = 0;
            g_mapMapNominationsCount.GetValue(prevNominatedMap, numNominations);

            numNominations--;

            if (numNominations <= 0)
            {
                g_mapMapNominationsCount.Remove(prevNominatedMap);
            }
            else
            {
                g_mapMapNominationsCount.SetValue(prevNominatedMap, numNominations, true);
            }
        }
    }

    g_mapClientNominatedMaps.SetString(clientBuffer, mapName, true);

    int newMapNumNominations = 0;

    if (g_mapMapNominationsCount.ContainsKey(mapName))
    {
        g_mapMapNominationsCount.GetValue(mapName, newMapNumNominations);
    }

    newMapNumNominations++;
    g_mapMapNominationsCount.SetValue(mapName, newMapNumNominations, true);

    RecalculateNominatedMapList();
}

public ArrayList GetTop3NominatedMaps()
{
    char   topMapNames[3][PLATFORM_MAX_PATH];
    int	   topMapVotes[3]  = { 0, 0, 0 };
    int	   minVotesInArray = 0;

    Handle hTrieSnapshot   = CreateTrieSnapshot(g_mapMapNominationsCount);
    char   szKey[PLATFORM_MAX_PATH];
    int	   iSize = TrieSnapshotLength(hTrieSnapshot);

    for (int i = 0; i < iSize; ++i)
    {
        GetTrieSnapshotKey(hTrieSnapshot, i, szKey, sizeof(szKey));
        int numVotes = 0;
        GetTrieValue(g_mapMapNominationsCount, szKey, numVotes);

        for (int b = 0; b < 3; b++)
        {
            if (numVotes > topMapVotes[b] && minVotesInArray == topMapVotes[b])
            {
                topMapVotes[b] = numVotes;
                strcopy(topMapNames[b], PLATFORM_MAX_PATH, szKey);

                break;
            }
        }

        int newMinVotesInArray = topMapVotes[0];
        for (int c = 1; c < 3; c++)
        {
            if (newMinVotesInArray > topMapVotes[c])
                newMinVotesInArray = topMapVotes[c];
        }

        minVotesInArray = newMinVotesInArray;
    }
    ...
    {
        if (topMapVotes[d] > 0)
            list.PushString(topMapNames[d]);
    }

    return list;
}

public void RecalculateNominatedMapList()
{
    ArrayList currentNominatedMaps = new ArrayList(PLATFORM_MAX_PATH);
    GetNominatedMapList(currentNominatedMaps);

    for (int i = 0; i < currentNominatedMaps.Length; i++)
    {
        char buffer[PLATFORM_MAX_PATH];
        currentNominatedMaps.GetString(i, buffer, PLATFORM_MAX_PATH);

        RemoveNominationByMap(buffer);
    }

    ArrayList top3Maps = GetTop3NominatedMaps();

    for (int i = 0; i < top3Maps.Length; i++)
    {
        char buffer[PLATFORM_MAX_PATH];
        top3Maps.GetString(i, buffer, PLATFORM_MAX_PATH);

        NominateMap(buffer, true, 0);
    }
}
